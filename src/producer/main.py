    ...
    def __init__(self):
        ...
        if self.kafka_username and self.kafka_password:
            self.producer_config.update({
                "security.protocol": "SASL_SSL",  #authentication and security layer
                "sasl.mechanism": "PLAIN",  #encrypted authntication with username and password
        ...
        self.compromised_users = set(random.sample(range(1000, 9999), 50))  #select 50 user ids
        self.high_risk_merchants = ['QuickCash', 'GlobalDigital', 'FastMoneyX']
        self.fraud_pattern_weights = {
            'account_takeover': 0.4,  # when someone stole password
            'card_testing': 0.3,  # 30% of fraud cases (test my card like if it has money or not)
...
if __name__ == "__main__":
    ...
    # producer.run_continuous_production(interval=0.5)  #create 2 transactions per second